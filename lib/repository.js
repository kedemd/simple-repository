        ...
        switch (currItem.action){
            case 'add' :
                self.adapter.create(currItemKey, currItem.data, function(err, item){
                    if (err) {
                        var error = new Error('Failed to create item while flushing');
            ...
            case 'remove' :
                self.adapter.remove(currItemKey, function(err){
                    if (err) {
                        var error = new Error('Failed to remove item while flushing');
                        error.action = 'remove';
                        error.key = currItemKey;
                        error.innerError = err;
                        return next(err);
            ...
            case 'update' :
                self.adapter.update(currItemKey, currItem.data, currItem.original, function(err, item){
                    if (err) {
                        var error = new Error('Failed to update item while flushing');
                        error.action = 'update';
                        error.key = currItemKey;
                        error.innerError = err;
